...
int process_symtab(int32_t sect_num,FILE *fp)
{
	...
	for(i=0;i<num_sym;i++)
	{
		...
		//printf(" %s ",sect_hdr.sh_name );
    	printf(" %lx ", mysym.st_value);

    	printf(" %lx ", mysym.st_size);

        printf(" %x ", mysym.st_info);

      ///  printf(" %x ", mysym.st_other);
...
int main(int argc, char *argv[])
{
    ...
    printf("text size: %i \n",text.size);
	 ...
	 hexDump (NULL, text.data, text.size);
/*
     j=0;
     k=0;
    ...
    printf("rodata size: %i \n",rodata.size);
    rodata.data=&buff[rodata.offset];
    hexDump (NULL, rodata.data, rodata.size);
    /*
    printf("%x\n",buff[rodata.offset]);
     j=0;
     k=0;
	...
	hexDump (NULL, data.data, data.size);
	/*
     j=0;
     k=0;
    ...
    hexDump (NULL, sdata.data, sdata.size);
    /*
     j=0;
     k=0;
