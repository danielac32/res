...
//extern	pid32	create(char *name,void (*entry)(void*),int nargs,void *args[]);
extern void sys_switch(void *ctx_old,void *ctx_new);

extern struct procent tasks[];
extern struct procent main_context;
extern struct procent *current_task;
extern int taskTop;  // total number of task
extern int counter_task;
...
extern syscall	getdev(char	*devname);
...
//extern syscall	signal(sid32		sem);
...
/* in file ramread.c */
extern	devcall	eeprom_read( struct dentry *, char *, uint32);

/* in file ramwrite.c */
extern	devcall	eeprom_write( struct dentry *, char *, uint32);

extern  devcall eeprom_seek ( struct dentry *devptr,uint32	offset);
...
//extern void avr_kprintf(const char me[]);
