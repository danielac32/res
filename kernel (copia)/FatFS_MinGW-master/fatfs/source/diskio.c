...
{
    memcpy(buff, &rambuf[sector*CFG_RAMDISK_SECTOR_SIZE], count *CFG_RAMDISK_SECTOR_SIZE);
    return 0;
}

int RAM_disk_write(const unsigned char *buff, unsigned int sector, unsigned int count)
{
    memcpy(&rambuf[sector*CFG_RAMDISK_SECTOR_SIZE], buff,  count *CFG_RAMDISK_SECTOR_SIZE);
    return 0;
}
